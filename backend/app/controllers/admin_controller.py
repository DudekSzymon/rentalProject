...
async def get_admin_dashboard(
    ...
    # Revenue tego miesiąca
    this_month_revenue = db.query(func.sum(Payment.amount)).filter(
        and_(
            Payment.status.in_([PaymentStatus.COMPLETED, PaymentStatus.OFFLINE_APPROVED]),
            Payment.processed_at >= datetime.now().replace(day=1)
        )
    ).scalar() or 0
...
async def get_pending_rentals(
    ...
    """Lista oczekujących wypożyczeń do zatwierdzenia"""
    
    query = db.query(Rental).filter(Rental.status == RentalStatus.PENDING)
    
    total = query.count()
    offset = (page - 1) * size
    rentals = query.order_by(Rental.created_at.asc()).offset(offset).limit(size).all()
...
async def get_pending_payments(
    ...
    """Lista oczekujących płatności do zatwierdzenia offline"""
    ...
    total = query.count()
    offset = (page - 1) * size
    payments = query.order_by(Payment.created_at.asc()).offset(offset).limit(size).all()
...
async def get_revenue_report(
    ...
    if not start_date:
        start_date = datetime.now().replace(day=1)  # Początek miesiąca
    ...
    # Najczęściej wypożyczany sprzęt
