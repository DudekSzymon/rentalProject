...
const StripePaymentForm = ({ rental, equipment, onPaymentSuccess, onPaymentError }) => {
    // Hooki Stripe do komunikacji z API i elementami formularza
    const stripe = useStripe();
    const elements = useElements();
    const { user } = useAuth(); // Dane zalogowanego użytkownika
    
    // Stany komponentu
    const [loading, setLoading] = useState(false);           // Stan ładowania podczas przetwarzania
    ...
    const createPaymentIntent = async () => {
        try {
            // Pobranie tokenu autoryzacyjnego z localStorage
            const token = localStorage.getItem('access_token');
            
            // Wysłanie żądania do backendu w celu utworzenia Payment Intent
            const response = await fetch('http://localhost:8000/api/payments/stripe/create-payment-intent', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}` // Autoryzacja JWT
                },
                body: JSON.stringify({
                    rental_id: rental.id,                
    ...
    const handleSubmit = async (event) => {
        ...
        setLoading(true);  // Włączenie stanu ładowania
        setError('');      // Wyczyszczenie poprzednich błędów
        ...
        try {
            ...
            if (error) {
                // Obsługa błędów płatności od Stripe
                setError(error.message);
                onPaymentError?.(error.message);
            } else if (confirmedPayment.status === 'succeeded') {
                // Płatność zakończona sukcesem
                await confirmPaymentInBackend(confirmedPayment.id); // Synchronizacja z backendem
                onPaymentSuccess?.(confirmedPayment); // Wywołanie callback'a sukcesu
            }
        } catch (err) {
    ...
    const confirmPaymentInBackend = async (paymentIntentId) => {
        try {
            ...
            // Wysłanie żądania potwierdzenia do backendu
            ...
            if (!response.ok) {
                //płatność przeszła w Stripe, ale nie udało się potwierdzić w backendzie
                console.warn('Błąd potwierdzania w backendzie, ale płatność przeszła');
            }
        } catch (err) {
            // Logowanie błędów komunikacji z backendem
            console.warn('Błąd potwierdzania w backendzie:', err);
        }
    };

    /**
     * Konfiguracja stylów dla elementu karty Stripe
     * Dostosowuje wygląd do ciemnego motywu aplikacji
     */
    const cardElementOptions = {
        style: {
            base: {
                ...
                '::placeholder': {
                    color: '#9ca3af',               // Szary placeholder
                },
                backgroundColor: 'transparent',      // Przezroczyste tło
            },
            invalid: {
                color: '#ef4444',                   // Czerwony kolor dla błędów
                iconColor: '#ef4444'
            }
        },
        hidePostalCode: true                        // Ukrycie pola kodu pocztowego
    };

    // Wyświetlenie loadera jeśli client secret nie jest jeszcze dostępny
    if (!clientSecret) {
        return (
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
                ...
                {/* Kontener dla elementu karty Stripe */}
            ...
            <Button
                type="submit"
                disabled={!stripe || loading} // Wyłączony gdy Stripe nie jest gotowy lub trwa ładowanie
            ...
            >
                {loading ? (
                    // Stan ładowania - animowany spinner
                    <>
                        <Loader2 className="w-5 h-5 mr-2 animate-spin" />
                        Przetwarzanie płatności...
                    </>
                ) : (
                    // Stan normalny - ikona karty i kwota
                    <>
 ...
 * Wyświetlany po pomyślnym zakończeniu transakcji
 */
            ...
            <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-6 border border-white/20 text-left space-y-4">
                ...
                <div className="flex items-center justify-between">
                    ...
                    <span className="text-white font-mono text-sm bg-white/10 px-3 py-1 rounded-lg">
                        {/* Wyświetlenie ostatnich 8 znaków ID płatności */}
...
const PaymentPage = () => {
    ...
    const [stripePromise, setStripePromise] = useState(null);     // Promise z załadowanym Stripe
    ...
    const handleOfflinePayment = () => {
        ...
        setPaymentData({ offline: true }); // Oznaczenie jako płatność offline
    };

    
    const handlePaymentSuccess = (paymentIntent) => {
        setPaymentSuccess(true);
        setPaymentData(paymentIntent); // Zapisanie danych płatności
    };

    /**
     * Obsługa błędów płatności
     * Callback wywoływany w przypadku błędu płatności
     */
    ...
    /**
     * Jeśli płatność zakończona sukcesem, wyświetl stronę sukcesu
     */
    if (paymentSuccess) {
        return (
            <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 relative overflow-hidden">
                {/* Animowane elementy tła dla sukcesu (zielone) */}
                <div className="absolute inset-0">
    ...
    /**
     * Główna strona płatności
     * Renderowanie formularza wyboru metody płatności i szczegółów
     */
    return (
        <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 relative overflow-hidden">
            ...
            {/* Główna zawartość - grid z formularzem płatności i podsumowaniem */}
            <div className="relative z-10 max-w-6xl mx-auto px-4 pb-12">
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    {/* Sekcja płatności (2/3 szerokości na desktop) */}
                    <div className="lg:col-span-2">
                        <Card className="bg-white/10 backdrop-blur-sm border border-white/20 rounded-2xl">
                            ...
                            <CardContent>
                                {/* Wybór metody płatności - przyciski */}
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
                                    {/* Przycisk płatności kartą */}
                                    <button
                                ...
                                {/* Renderowanie odpowiedniego formularza płatności */}
                                {paymentMethod === 'card' && stripePromise ? (
                                    // Formularz płatności kartą (Stripe Elements)
                                    ...
                                    // Formularz płatności offline
                                    <div className="space-y-6">
                                        {/* Informacje o płatności offline */}
                                        ...
                                        {/* Przycisk potwierdzenia płatności offline */}
                                        <Button 
                                    ...
                                    // Stan ładowania - gdy Stripe się jeszcze ładuje
                                ...
                                {/* Wyświetlanie globalnych błędów */}
                                {error && (
                    ...
                    {/* Sidebar z podsumowaniem (1/3 szerokości na desktop) */}
                    <div className="space-y-6">
                        {/* Karta z informacjami o sprzęcie */}
                        <Card className="bg-white/10 backdrop-blur-sm border border-white/20 rounded-2xl">
                            <CardHeader>
                            ...
                            <CardContent className="space-y-6">
                                ...
                                {/* Informacje o okresie wypożyczenia */}
                                    ...
                                    <div className="space-y-2 text-sm">
                                        <div className="flex justify-between">
                                            ...
                                            <span className="text-white font-medium">
                                                {/* Formatowanie daty na format polski */}
                                ...
                                <div className="border-t border-white/20 pt-4 space-y-3">
                                    ...
                                    {/* Wyświetlenie kaucji jeśli jest wymagana */}
                        ...
                        {/* Karta z informacjami o bezpieczeństwie płatności */}
                        <Card className="bg-white/10 backdrop-blur-sm border border-white/20 rounded-2xl">
