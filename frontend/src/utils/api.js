      ...
      if (refreshToken) {
        try {
          ...
          const response = await axios.post(
            ...
            {
              refresh_token: refreshToken, // <- Refresh token w body
            }
          );
          ...
          // Zapisz nowe tokeny
          localStorage.setItem("access_token", access_token); // <- Nowy Acess (30)
          localStorage.setItem("refresh_token", newRefreshToken); // <- Ten sam refresh
...
export const authAPI = {
  ...
  //Frontend -> Backend: TOKEN w BODY-POST
...
export const rentalsAPI = {
  ...
  checkAvailability: (params) =>
    api.get("/api/rentals/check-availability", { params }),
  getPricingPreview: (params) =>
    api.get("/api/rentals/pricing-preview", { params }),
};
...
export const paymentsAPI = {
  ...
  createStripeIntent: (data) =>
    api.post("/api/payments/stripe/create-payment-intent", data),
  confirmStripe: (paymentIntentId) =>
    api.post(`/api/payments/stripe/confirm/${paymentIntentId}`),
...
export const adminAPI = {
  ...
  getPendingRentals: (params = {}) =>
    api.get("/api/admin/rentals/pending", { params }),
  getRevenueReport: (params = {}) =>
    api.get("/api/admin/reports/revenue", { params }),
  ...
  createOfflinePayment: (data) =>
    api.post("/api/payments/create-offline", data),
};
